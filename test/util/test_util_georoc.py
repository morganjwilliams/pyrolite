...
class TestParseValues(unittest.TestCase):
    ...
    def test_sub(self):
        for val, sub, expect in [('A [1]', lambda x: x, 'A'),
...
class TestParseCitations(unittest.TestCase):
    ...
    def test_series(self):
        for val, expect in [(pd.Series(['[1] A']),
                             pd.Series([{'value':'A', 'key': '1'}])),
                             ]:
            with self.subTest(val=val, expect=expect):
...
class TestParseDOI(unittest.TestCase):
    ...
    def test_series(self):
        for val, expect in [(pd.Series(['[1] A doi:10.x01/g']),
                             pd.Series(['dx.doi.org/10.x01/g'])),
                             ]:
            with self.subTest(val=val, expect=expect):
                self.assertTrue((parse_DOI(val)==expect).all())

    def test_link(self):
        for val, link, expect in [('[1] A doi:10.x01/g', True,
                                    'dx.doi.org/10.x01/g'),
                                  ('[1] A doi:10.x01/g', False,
                                     '10.x01/g')
                            ]:
...
class TestDownloadGEOROCCompilation(unittest.TestCase):
    ...
    def test_dataframe_return(self):
        bulk_GEOROC_download(output_folder=self.temp_dir,
                             reservoirs=self.res)
